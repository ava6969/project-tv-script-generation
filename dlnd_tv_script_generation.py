...
def token_lookup():
    ...
    token_dict['('] = '<LEFT_PARANTHESES>'
    token_dict[')'] = '<RIGHT_PARANTHESES>'
    token_dict['-'] = '<DASH>'
    token_dict['\n'] = '<RETURN>'
    return token_dict
...
def batch_data(words, sequence_length, batch_size):
    ...
    features = np.zeros((n_targets, sequence_length), dtype=int)
    target = np.zeros(n_targets, dtype=int)
    for i in range(n_targets):
        features[i] = words[i : i+sequence_length]
        target[i]   = words[i+sequence_length]
...
# Data params
# Sequence Length
sequence_length = 35  # of words in a sequence
# Batch Size
batch_size = 256
...
# **Answer:** i only changed the number of epochs, embeddeding dim and lstm hidden dim and i was able to reach the required loss
